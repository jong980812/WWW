...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj_drop(x)
        return x
    def get_each_attn(self,x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)
        attn = torch.zeros(B, q.shape[1], C // self.num_heads, 1, N-1,device=q.device)
        for d in range(C // self.num_heads):
            q_d = q[:, :, :, d].unsqueeze(-1)  # q_d.shape: [batch_size, num_head, seq_len]
            k_d = k[:, :, :, d].unsqueeze(-1)  # k_d.shape: [batch_size, num_head, seq_len]
            attn_result =(q_d @ k_d.transpose(-2, -1)) * self.scale
            
            attn[:, :, d, :, :] = attn_result[:,:,0:1,1:]  # 각 dim별로 Attention Score 계산

        # 결과 출력
        return attn  # 예상: [batch_size, num_head, dim, seq_len, seq_len]
...
class Block(nn.Module):
    ...
    def get_attn_weights(self,x):
        attn = self.attn.get_each_attn(self.norm1(x))
        B,h,d,_,N = attn.shape
        attn = rearrange(attn.squeeze(-2), 'b h d n -> b (h d) n',b=B,n=N)
        return attn.mean(-1)
    def forward(self, x):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
...
class VisionTransformer(nn.Module):
    ...
    def reset_classifier(self, num_classes: int, global_pool=None, p=None, info=None,
                         clip_threshold=1e10, LU = True):
        self.num_classes = num_classes
    ...
    def _compute_taylor_scores(self, inputs,labels,target_layer):
        ...
        msp = np.max(logits)
        outputs[0, pred_label].backward(retain_graph=True)
        ...
        self.remove_handles()
        return first_order_taylor_scores, pred_label, msp
    def extract_feature_patch_embed(self,img):
        x = self.patch_embed(img)
        x=self.avgpool(x.transpose(-1,-2)).flatten(1)
        return x#Batch,dim
    def extract_cls_feature(self,x,layer):
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.norm_pre(x)
        for i in range(layer):
            x = self.blocks[i](x)
        # cls_feat = self.blocks[layer].get_cls(x)
        cls_feat = self.blocks[layer].get_attn_weights(x)
        return cls_feat
    def extract_cls_feature_prev_mlp(self,x,layer):
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.norm_pre(x)
        for i in range(layer):
    ...
    def _hook_layers(self,layer):
        if layer.split('_')[0]=='cls':
            cls=True
            layer = layer[4:]
        else:
            cls=False
        def backward_hook_relu(module, grad_input, grad_output):
            grad = grad_output[0][:,0,:] if cls else grad_output[0] 
            self.gradients.append(grad.to(self.device))
        ...
        def forward_hook_relu(module, input, output):
            ...
            out = output[:,0,:] if cls else output[0]
            self.activations.append(out.to(self.device))
            return output
