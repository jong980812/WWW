...
def parse_args():
    ...
    parser.add_argument('--example_root',type=str, default='./datasets/ILSVRC-2012/val', help='Path to D_probe')
    parser.add_argument('--heatmap_save_root',type=str, default='./heatmap', help='Path to saved img')
    parser.add_argument('--num_example', default=50, type=int, help='# of examples to be used')
    parser.add_argument('--shapley_root', default='./utils',type=str, help='Path to utils')
    parser.add_argument('--map_root', default='./heatmap_info', type=str,help='Path to utils')
...
def concept_attribution_maps(cmaps, args, model, example_loader, num_top_neuron=5, percentile=90, alpha=0.7, gt=False):
    ...
    with open(args.shapley_root, "rb") as f:
        shap_value = pkl.load(f) # 1000*768
    ...
    for j, (img, label) in enumerate(example_loader):
        ...
        if len(os.listdir(f'{args.heatmap_save_root}/{label.item():04d}/class_attribute_n{num_top_neuron}_p{percentile}_a90'))>args.num_example:
            continue
        show(img[0])        
        img = img.cuda()    
        feature_maps = model.extract_cls_feature_prev_mlp(img,0) # 1*3*224*224 => 1*2048*7*7
        ...
        feature_maps = feature_maps[0,1:,:].cpu().detach().numpy()
    ...
    for j, (img, label) in enumerate(example_loader):
        ...
        if len(os.listdir(f'{args.heatmap_save_root}/{label.item():04d}/class_overall_n{num_top_neuron}_p{percentile}_a90'))>args.num_example:
            continue
        show(img[0]) 
        img = img.cuda()    
        feature_maps = model.extract_cls_feature_prev_mlp(img,0)
        feature_maps = feature_maps[0,1:,:].cpu().detach().numpy()
        feature_maps = feature_maps.reshape(14,14,-1)#14,14,768
        predict = model(img)
        predict = predict[0].cpu().detach().numpy()
    ...
    for j, (img, label) in enumerate(example_loader): # len(example_loader) = 2800 (그래서 sc_idx 읽어보면 len():2800임)
        ...
        if len(os.listdir(f'{args.heatmap_save_root}/{label.item():04d}/sample_attribute_n{num_top_neuron}_p{percentile}_a90'))>args.num_example:
            continue
        show(img[0])
        img = img.cuda() # 1*3*224*224
        feature_maps = model.extract_cls_feature_prev_mlp(img,0) # 1*2048*7*7
        predict = model(img) 
    ...
    for j, (img, label) in enumerate(example_loader):
        ...
        if len(os.listdir(f'{args.heatmap_save_root}/{label.item():04d}/sample_overall_n{num_top_neuron}_p{percentile}_a90'))>args.num_example:
            continue
        show(img[0])
        img = img.cuda()    
        feature_maps = model.extract_cls_feature_prev_mlp(img,0)
